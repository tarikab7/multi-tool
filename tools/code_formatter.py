...
def format_xml(code, indent):
    # Using BeautifulSoup to format HTML/XML cleanly
    soup = BeautifulSoup(code, 'html.parser')
    return soup.prettify()

def format_css(code, indent):
    # Simple regex-less CSS formatter: format brackets and rules
    lines = []
    indent_str = " " * indent
    # Strip existing spacing
    code = code.replace("\n", "").replace("\r", "")
    parts = code.split("}")
...
def format_sql(code):
    # Capitalize main SQL keywords and format newlines
    ...
    code = " ".join(code.split()) # normalize spacing
    for kw in keywords:
        # Replace case-insensitive keyword with capitalized keyword and a newline prefix
        # Use simple word replacement
...
async def run(params: dict):
    ...
    try:
        ...
        if lang == "json":
            ...
            import re  # import here for sql helper
