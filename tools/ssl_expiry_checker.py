...
async def run(params: dict):
    ...
    try:
        ...
        if expiry_str:
            ...
            yield ToolEvent(type="found", message=f"Issuer: {dict(x[0] for x in cert.get('issuer', []))}")
            yield ToolEvent(type="found", message=f"Expires On: {expiry_str}")
            yield ToolEvent(type="found", message=f"Days Remaining: {days_left} days")
