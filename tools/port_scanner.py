...
async def scan_single_port(host, port):
    try:
        # Try to establish connection with 1-second timeout
...
async def run(params: dict):
    ...
    # Parse port ranges (supports ranges like '20-100' or lists like '80,443')
    ports = []
    try:
        if "-" in range_str:
    ...
    # Bound ports count to keep safety
    if len(ports) > 2000:
    ...
    # Run port checks concurrently in batches to avoid OS descriptor exhaustion
    batch_size = 100
    open_ports = []
    scanned_count = 0
