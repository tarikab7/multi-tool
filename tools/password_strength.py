...
async def run(params: dict):
    ...
    try:
        ...
        yield ToolEvent(event_type="found", message=f"Length: {length}")
        yield ToolEvent(event_type="found", message=f"Alphabet pool size: {pool}")
        yield ToolEvent(event_type="found", message=f"Entropy: {entropy:.2f} bits")
        yield ToolEvent(event_type="found", message=f"Strength: {strength}")
