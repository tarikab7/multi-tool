...
async def run(params: dict):
    ...
    try:
        ...
        yield ToolEvent(type="found", message=f"Length: {length}")
        yield ToolEvent(type="found", message=f"Alphabet pool size: {pool}")
        yield ToolEvent(type="found", message=f"Entropy: {entropy:.2f} bits")
        yield ToolEvent(type="found", message=f"Strength: {strength}")
