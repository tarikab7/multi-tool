...
async def run(params: dict):
    ...
    try:
        step1_success = False
        async for event in run_ffmpeg_with_progress(cmd1, duration, "Step 1 complete"):
            if event["type"] == "progress":
                # Scale progress to 0-50%
                yield {"type": "progress", "progress": event["progress"] / 2.0}
        ...
        async for event in run_ffmpeg_with_progress(cmd2, duration, f"Stabilized video written to: {output_path}"):
            if event["type"] == "progress":
                # Scale progress to 50-100%
                yield {"type": "progress", "progress": 50.0 + (event["progress"] / 2.0)}
            else:
                yield event
    finally:
