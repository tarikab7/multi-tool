...
async def test_run_valid_path(temp_dir_structure):
    ...
    # 1 mapping log + 1 base dir log + 4 lines + 1 success = 7 results
...
async def test_run_truncation():
    with tempfile.TemporaryDirectory() as temp_dir:
        # Patch make_tree to return > 300 lines
        ...
        with patch("tools.directory_tree.make_tree", return_value=fake_lines):
            ...
            # 1 mapping log + 300 line logs + 1 truncation log = 302 logs
