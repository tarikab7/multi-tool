...
async def test_word_counter_success():
    ...
    assert any(r.get("type") == "log" and "Tokenizing text" in r.get("message", "") for r in results)
    assert any(r.get("type") == "found" and "Characters: 11" in r.get("message", "") for r in results)
    assert any(r.get("type") == "found" and "Words: 2" in r.get("message", "") for r in results)
    assert any(r.get("type") == "found" and "Lines: 1" in r.get("message", "") for r in results)
...
async def test_json_minifier_format():
    ...
    # Check if formatted JSON is in the results
    found_event = next((r for r in results if r.get("type") == "found"), None)
    assert found_event is not None
    assert '"a": 1' in found_event["message"]
    assert "\n" in found_event["message"]
...
async def test_json_minifier_minify():
    ...
    # Check if minified JSON is in the results
    found_event = next((r for r in results if r.get("type") == "found"), None)
    assert found_event is not None
    assert found_event["message"] == '{"a":1,"b":2}'

    assert any(r.get("type") == "success" for r in results)

@pytest.mark.asyncio
async def test_json_minifier_invalid():
    params = {"raw_json": '{"a": 1,', "action": "minify"}
    results = [res async for res in json_minifier.run(params)]
